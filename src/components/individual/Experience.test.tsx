...
describe('Experience List component rendering', () => {
    ...
    it('should render all experience information in separate elements', () => {
        ...
        sectionContentSchool.forEach((instance, index) => {
            ...
            expect(instance.findByProps({ className: 'experience-date-label' }).children).toStrictEqual([`${schoolExperience[index].fromDate} - ${schoolExperience[index].toDate}`]);
            expect(instance.findByProps({ className: 'experience-city-label' }).children).toStrictEqual([schoolExperience[index].city]);
            expect(instance.findByProps({ className: 'experience-role-label' }).children).toStrictEqual([schoolExperience[index].role]);
            expect(instance.findByProps({ className: 'experience-summary-label' }).children).toStrictEqual([schoolExperience[index].summary]);
        });
        sectionContentWork.forEach((instance, index) => {
            ...
            expect(instance.findByProps({ className: 'experience-date-label' }).children).toStrictEqual([`${workExperience[index].fromDate} - ${workExperience[index].toDate}`]);
            expect(instance.findByProps({ className: 'experience-city-label' }).children).toStrictEqual([workExperience[index].city]);
            expect(instance.findByProps({ className: 'experience-role-label' }).children).toStrictEqual([workExperience[index].role]);
            expect(instance.findByProps({ className: 'experience-summary-label' }).children).toStrictEqual([workExperience[index].summary]);
        });
    });
});
