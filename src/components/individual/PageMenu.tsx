    ...
    return (
        <div className={`page-menu-list-item ${props.type}`}>
            <Link 
                ...
                <svg id={`paddle-icon-${target}`} xmlns='http://www.w3.org/2000/svg' className={`bi bi-${info.iconClass}`} viewBox='0 0 16 16'>
                    {info.iconPaths.map((iconPath, index) => <path d={iconPath} key={`paddle-icon-path-${index}`} />)}
                </svg>
